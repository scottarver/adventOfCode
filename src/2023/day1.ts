...
const part1TestInput = `1abc2
pqr3stu8vwx
a1b2c3d4e5f
treb7uchet`;

function firstLastSum(input: string) {
    // split the input into lines
    const input1Lines = input.split('\n');
    // remove all non-numbers
    const onlyNubmers = input1Lines.map((line) => line.replace(/[^0-9]/g, ''));
    // split each line into an array of single digits
    const singles = onlyNubmers.map((line) => line.split('').map(Number));
    const onlyFirstAndLasts = singles.map((line) => [line[0], line[line.length - 1]]);
    const recombined = onlyFirstAndLasts.map((line) => line.join('')).map(Number);
    const sum = recombined.reduce((acc, curr) => acc + curr, 0);
    console.log(sum);
}
...
numbers.forEach(([word, number]) => numbersMap.set(word, number));
const numbersText = numbers.map(([word]) => word);

export const positionToEnd = (input: string, position: number) => {
    return input.slice(position);
};

// given the string "one", return 1
// given the string "1one", return 11
const replaceAtStart = (input: string, wordToReplace: string, replaceWith: string) => {
    return input.replace(new RegExp(`^${wordToReplace}`), replaceWith);
};
// console.assert(replaceAtStart('bobSteve', 'bob', '1') === '1Steve', 'replaceAtStart 1');
// console.log(replaceAtStart('bobSteve', 'bob', '1'));;

export const splitWordAtPosition = (word: string, position: number): [string, string] => {
...
// given the string "one", return [1]
// two1nine
// given the string "two1nine" return [2, 1, 9]
// eightwothree
// given the string "eightwothree" return [8, 2, 3]
// abcone2threexyz
// given the string "abcone2threexyz" return [1, 2, 3]
// xtwone3four
// given the string "xtwone3four" return [1, 3, 4]
// 4nineeightseven2
// given the string "4nineeightseven2" return [4, 9, 8, 7, 2]
// zoneight234
// given the string "zoneight234" return [1, 8, 2, 3, 4]
// 7pqrstsixteen
// given the string "7pqrstsixteen" return [7, 6]
export const findNumbersByText = (input: string): number[] => {
    // scan thru the input string, looking for the first number as text
    // if found, replace it with the number
    // repeat until no more numbers are found or we hit the end of the string
    // return the resulting string
    let workingString = input;
    let ret = [];
    for (let i = 0; i < workingString.length; i++) {
        const [before, after] = splitWordAtPosition(workingString, i);
        const maybeNum = workingString[i];
        // if it is 0-9, add it to the return array
        if (/[0-9]/.test(maybeNum)) {
            ret.push(Number(maybeNum));
            continue;
        }
        const found = numbers.some(([word, number]) => {
...
const onlyFirstAndLasts = (input: number[]): [number, number] => {
    const first = input[0];
    const last = input[input.length - 1];
    return [first, last];
};

firstLastSum(part1TestInput);

const part1RealInput = `2911threeninesdvxvheightwobm
...
zoneight234
7pqrstsixteen`;
const p2TLines = part2TestInput.split('\n');
const p2ToNumbers = p2TLines.map((line) => findNumbersByText(line));
console.log(p2ToNumbers);
const p2FirstAndLasts = p2ToNumbers.map((numbers) => onlyFirstAndLasts(numbers));
console.log(p2FirstAndLasts);
const asNumbers = p2FirstAndLasts.map((numbers) => parseInt(numbers.join(''),10));
console.log(asNumbers);
const sum = asNumbers.reduce((acc, curr) => acc + curr, 0);
console.log(sum);

const part2 = (input: string): number => {
   const p2TLines = input.split('\n');
   const p2ToNumbers = p2TLines.map((line) => findNumbersByText(line));
//    console.log(p2ToNumbers);
   const p2FirstAndLasts = p2ToNumbers.map((numbers) => onlyFirstAndLasts(numbers));
//    console.log(p2FirstAndLasts);
   ...
   console.log(asNumbers);
    ...
    const joined = zip(p2TLines, p2ToNumbers, p2FirstAndLasts, asNumbers);
    joined.forEach((line) => console.log(line));
    // console.log(joined);
...
console.log(part2(part2TestInput));
console.log(part2(part1RealInput));
